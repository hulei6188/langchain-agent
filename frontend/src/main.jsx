...
import {
  CHAT_WELCOME_SNAPSHOT_STORAGE_KEY,
...
function App() {
  ...
  const runningSessionKeySignature = useMemo(
    () => Object.keys(runningBySessionId)
      .filter((key) => runningBySessionId[key]?.running)
      .sort()
      .join('|'),
    [runningBySessionId],
  );
  ...
  useEffect(() => {
    if (!token || !activeAgentId || !runningSessionKeySignature) return undefined;
    let disposed = false;
    const pollRunningSessions = () => {
      if (disposed || !activeAgentIdRef.current) return;
      loadSessions(activeAgentIdRef.current).catch((err) => {
        if (!disposed) console.error(err);
      });
    };
    pollRunningSessions();
    const timer = window.setInterval(pollRunningSessions, 1500);
    return () => {
      disposed = true;
      window.clearInterval(timer);
    };
  }, [token, activeAgentId, runningSessionKeySignature]);
  ...
  function clearSessionRunState(sessionId, options = {}) {
    const key = sessionCacheKey(sessionId);
    if (!key) return;
    clearSessionRunning(key);
    clearReasoningIdleTimer(key);
    const state = sessionRunRef.current[key];
    if (options.abort !== false) {
      state?.controller?.abort();
    }
    delete sessionRunRef.current[key];
  }
  ...
  async function refreshSessionCache(sessionId) {
    const data = await api(`/api/sessions/${sessionId}`, { token });
    const loaded = data.messages || [];
    const activeRun = data.active_run;
    const visible = isSessionKeyVisible(sessionId);
    writeSessionMessages(sessionId, loaded, { visible });
    writeSessionSources(sessionId, latestSourcesFromMessages(loaded), { visible });
    if (visible) {
      setSessionTitleDraft(data.session?.title || '');
    }
    if (activeRun?.status === 'running') {
    ...
    if (data.session) {
      setSessions((prev) => prev.map((session) => (
        sameRouteId(session.id, sessionId)
          ? { ...session, ...data.session, active_run: activeRun }
          : session
      )));
    }
    return data;
  ...
  async function reconnectToRun(runId, sessionId, existingMessages) {
    ...
    try {
      ...
      if (sessionRunRef.current[sessionId]?.genId === genId) {
        ...
        if (streamEnded) {
          refreshSessionCache(sessionId).catch(() => {});
        }
      }
  ...
  function handleSse(raw, ctx) {
    ...
    if (event === 'cancelled') {
      ...
      if (data.session_id) {
        setSessions((prev) => prev.map((session) => (
          sameRouteId(session.id, data.session_id) ? { ...session, active_run: null } : session
        )));
      }
      if (visible) {
    ...
    if (event === 'done') {
      ...
      // Immediately update session title and clear active run in sidebar.
      if (data.session_id) {
        setSessions((prev) =>
          prev.map((s) => (sameRouteId(s.id, data.session_id) ? { ...s, ...(data.title ? { title: data.title } : {}), active_run: null } : s))
        );
      }
      if (visible) {
        if (ctx?.isNewSession && data.session_id && !activeSessionIdRef.current) {
